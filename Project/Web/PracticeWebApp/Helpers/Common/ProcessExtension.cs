...
namespace PracticeWebApp.Helpers.Common
{
    public static class ProcessExtension
    {
        ...
        public static IEnumerable<Process> GetChildProcessesByName(this Process process, string processName)
        {
            var processes = Process.GetProcessesByName(processName);
            foreach (var childProcess in processes)
            {
                Process childParentProcess = null;
                try { childParentProcess = childProcess.GetParentProcess(); }
                catch { }

                if (childParentProcess?.Id == process.Id)
